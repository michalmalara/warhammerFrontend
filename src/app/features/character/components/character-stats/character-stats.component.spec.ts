...
describe('CharacterStatsComponent.statTotal', () => {
  ...
  it('for primary calculates base + adv*5', () => {
    expect(component.statTotal({label: 'WS', base: 30, adv: 3}, 'primary')).toBe(45);
  });

  it('for secondary calculates base + adv (no multiplier)', () => {
    expect(component.statTotal({label: 'Wounds', base: 30, adv: 3}, 'secondary')).toBe(33);
  });

  it('when total is provided it takes precedence (does not add adv)', () => {
  ...
  it('missing base/adv are treated as 0', () => {
