...
export class CharacterCreationStep5FinalReviewWealthComponent {
  ...
  commitCharacter = async () => {
    ...
    if (!p) {
      // jeśli nie ma profesji, wróć do kroku 3
    ...
    try {
      ...
      // Minimalny zapis: tworzymy Character (wymaga currantProfession i characterProfile).
      // Profile tworzy backend? Jeśli nie, trzeba to spiąć w kolejnych iteracjach.
      // Na teraz przyjmujemy, że istnieje profile o ID=1 (dev) — w realnym flow krok 2/3 powinien go tworzyć.
      // Żeby nie blokować UI, zapisujemy jako draft kiedy profile/brak.
      const payload: CharacterCreatePayload = {
        ...
        eyes: b.eyeColor,
        hair: b.hairColor,
        currantProfession: p.id,
        characterProfile: 1,
        equipment: (p.trappings ?? '').trim(),
        goldCrowns: this.goldCrowns() ?? undefined,
      };
